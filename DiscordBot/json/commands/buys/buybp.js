...
module.exports = {
  name: 'buyblackpanther',
  description: 'buy black panther command',
 async execute(message, args){

    const embed = new Discord.MessageEmbed()
    embed.setTitle('Do you want to buy Black Panther?')
    ...
    embed.setDescription("Click on the 💰 to buy.")
        ...
        buyCollector.on('collect', async (reaction, user) => {
            if (reaction.emoji.name === '💰') {
                message.channel.send("You have just bought Black Panther!")
            } 
        });
    }
}