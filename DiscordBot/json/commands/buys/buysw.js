...
module.exports = {
  name: 'buyscarletwitch',
  description: 'buy scarlet witch command',
 async execute(message, args){

    const embed = new Discord.MessageEmbed()
    embed.setTitle('Do you want to buy Scarlet Witch?')
    ...
    embed.setDescription("Click on the 💰 to buy.")
        ...
        buyCollector.on('collect', async (reaction, user) => {
            if (reaction.emoji.name === '💰') {
                message.channel.send("You have just bought Scarlet Witch!")
            } 
        });
    }
}