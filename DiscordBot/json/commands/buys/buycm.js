...
module.exports = {
  name: 'buycaptainmarvel',
  description: 'buy captain marvel command',
 async execute(message, args){

    const embed = new Discord.MessageEmbed()
    embed.setTitle('Do you want to buy Captain Marvel?')
    ...
    embed.setDescription("Click on the 💰 to buy.")
        ...
        buyCollector.on('collect', async (reaction, user) => {
            if (reaction.emoji.name === '💰') {
                message.channel.send("You have just bought Captain Marvel!")
            } 
        });
    }
}